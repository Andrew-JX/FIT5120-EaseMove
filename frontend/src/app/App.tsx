...
import {
  Thermometer, Droplets, Users, X, Home, Wind, Map,
...
function LayerSwitcherPanel({
  ...
  onSelect: (layer: 'ease-places' | 'comfort-area') => void;
}) {
  return (
    <div className="map-right-panel">
      <div className="map-panel-header">Layers</div>
      {(
        [
          { id: 'ease-places', label: 'Ease Places' },
          { id: 'comfort-area', label: 'Comfort Area' },
        ] as const
      ).map(({ id, label }) => (
        <button
          key={id}
          type="button"
          className={`layer-item${activeLayer === id ? ' active' : ''}`}
          onClick={() => onSelect(id)}
        >
          <div className={`layer-radio${activeLayer === id ? ' checked' : ''}`} />
          {label}
        </button>
      ))}
    </div>
  ...
  return (
    <>
      ...
      <div className="legend-row">
        <div className="furniture-dot furniture-dot-bin" />
        <span>Bin</span>
      </div>
      <div className="legend-row">
...
function LegendPanel({ activeLayer }: { activeLayer: 'ease-places' | 'comfort-area' }) {
  return (
    <div className="map-right-panel">
...
export default function App() {
  ...
  // Layer state — default is Ease Places; Comfort Area data loads lazily on switch
  const [activeLayer, setActiveLayer] = useState<'ease-places' | 'comfort-area'>('ease-places');
  const comfortLayerActive = activeLayer === 'comfort-area';

  const { precincts: precinctList, loading, error } = usePrecincts(
    comfortLayerActive ? debouncedWeights : undefined
  );
  ...
  // Left sidebar starts collapsed — only relevant when Comfort Area is active
  ...
  const handleLayerSwitch = useCallback((layer: 'ease-places' | 'comfort-area') => {
    setActiveLayer(layer);
    setOpenPanel(null); // close layer switcher after selection
    setEasePlacesPopup(null);
    setShowCard(null);
    if (layer === 'comfort-area') {
      setSidebarCollapsed(false); // auto-expand left sidebar for Comfort Area
    } else {
  ...
  const getComparisonRecommendation = () => {
    ...
    if (riskPriority[r2] > riskPriority[r1])
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <h2 className="text-2xl font-semibold bg-gradient-to-r from-teal-600 to-cyan-600 bg-clip-text text-transparent">
                        {activeLayer === 'ease-places' ? 'Ease Places' : 'Precinct Map View'}
                      </h2>
                      <p className="text-sm text-gray-600">
                        {activeLayer === 'ease-places'
                          ? 'Explore cool spots — click a marker for details'
                          : 'Interactive map — click a marker for detailed score card'}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex">
                  ...
                  <div
                    className={`border-r border-gray-200 bg-gray-50/50 transition-all duration-300 overflow-x-hidden overflow-y-auto ${
                      sidebarCollapsed || activeLayer === 'ease-places' ? 'w-0' : 'w-56'
                    }`}
                  ...
                  <div className="flex-1 relative">
                    {/* Left sidebar toggle — only visible in Comfort Area mode */}
                    {activeLayer === 'comfort-area' && (
                      <button
                        type="button"
                    ...
                    {openPanel === 'layers' && (
                      <LayerSwitcherPanel activeLayer={activeLayer} onSelect={handleLayerSwitch} />
                    )}
                    {openPanel === 'legend' && (
                    ...
                    <LeafletMap
                      precincts={comfortLayerActive ? precinctList : []}
                      selectedCategory={comfortLayerActive ? selectedCategory : null}
                      activeMode="view"
                    ...
                    {/* Ease Places popup */}
                    {easePlacesPopup && activeLayer === 'ease-places' && (
                      <EasePlacesPopup
                    ...
                    {showCard && showCardPrecinct && activeLayer === 'comfort-area' && (
